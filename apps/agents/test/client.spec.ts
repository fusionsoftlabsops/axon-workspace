...
describe('AxonApi', () => {
  ...
  it('marketingKit pega al endpoint marketing de la HU', async () => {
    fetchMock.mockResolvedValue(jsonResponse({ ok: true, marketing: { kit: 'copy', assetFileId: 'f1' } }, 201));
    const res = await api.marketingKit('axon', 60);
    const [url, init] = fetchMock.mock.calls[0]!;
    expect(url).toContain('/projects/axon/tasks/60/marketing');
    expect(init.method).toBe('POST');
    expect(res.marketing.assetFileId).toBe('f1');
  });
