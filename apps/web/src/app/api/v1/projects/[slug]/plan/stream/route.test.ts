...
vi.mock('@/lib/db', () => ({
  prisma: { projectPlan: { findFirst: h.planFindFirst }, user: { findUnique: h.userFindUnique } },
}));
...
beforeEach(() => {
  ...
  h.userFindUnique.mockResolvedValue({ name: 'Me' });
});

describe('GET plan stream', () => {
  ...
  it('404 when no plan exists', async () => {
    h.planFindFirst.mockResolvedValue(null);
    expect((await GET(new NextRequest('http://localhost/x'), ctx)).status).toBe(404);
  });

  it('200 opens an SSE stream and subscribes', async () => {
    ...
    expect(h.subscribe).toHaveBeenCalledWith('plan:pl1', expect.any(Function));
    expect(h.publish).toHaveBeenCalledWith(
      'plan:pl1',
      expect.objectContaining({ type: 'presence', state: 'join', userId: 'u1' }),
    );
    await reader.cancel();
  });
});
