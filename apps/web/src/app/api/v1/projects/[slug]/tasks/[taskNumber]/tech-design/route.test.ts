...
vi.mock('@/lib/agents/architect', () => ({ designTaskArchitecture: h.designTaskArchitecture }));

import { POST } from './route';

const ctx = { params: Promise.resolve({ slug: 'proj', taskNumber: '50' }) };
...
describe('POST tech-design', () => {
  ...
  it('201 designs + audits', async () => {
    h.designTaskArchitecture.mockResolvedValue('## Arquitectura');
    const res = await POST(req({ lang: 'es' }), ctx);
    expect(res.status).toBe(201);
    expect((await res.json()).design).toContain('Arquitectura');
    expect(h.designTaskArchitecture).toHaveBeenCalledWith(
      expect.objectContaining({ projectId: 'p1', taskId: 'task-40', actorUserId: 'u1' }),
    );
    expect(h.audit).toHaveBeenCalledWith(expect.objectContaining({ action: 'task.tech_design' }));
  });
  it('502 when generation fails', async () => {
    h.designTaskArchitecture.mockRejectedValue(new Error('IA caída'));
