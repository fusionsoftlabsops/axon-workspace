...
vi.mock('@/lib/brain', () => ({ citeMemory: h.citeMemory }));
vi.mock('@/lib/db', () => ({
  prisma: {
...
describe('POST cite', () => {
  ...
  it('400 when citeMemory fails', async () => {
    h.projectFindUnique.mockResolvedValue({ id: 'p1', members: [{ role: 'ADMIN' }] });
    h.taskFindUnique.mockResolvedValue({ id: 't1' });
    h.citeMemory.mockResolvedValue({ ok: false, error: 'bad' });
    expect((await POST(req({ taskNumber: 1 }), ctx)).status).toBe(400);
  });
  it('201 cites a memory', async () => {
    h.projectFindUnique.mockResolvedValue({ id: 'p1', members: [{ role: 'ADMIN' }] });
    h.taskFindUnique.mockResolvedValue({ id: 't1' });
    h.citeMemory.mockResolvedValue({ ok: true, citationId: 'c1' });
    const res = await POST(req({ taskNumber: 1, context: 'why' }), ctx);
    expect(res.status).toBe(201);
    expect(await res.json()).toEqual({ ok: true, citationId: 'c1' });
