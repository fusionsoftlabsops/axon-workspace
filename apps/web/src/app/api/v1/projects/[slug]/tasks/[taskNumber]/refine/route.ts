...
import { refineTaskForReadiness } from '@/lib/agents/refine';
 ...
 * marca lista (publica `story.refined`). Lo llama el agente Product Owner.
 */
export async function POST(
  ...
  if (found.role === 'VIEWER') {
    return NextResponse.json({ error: 'viewer cannot refine' }, { status: 403 });
  }
  ...
  let refinement;
  try {
    refinement = await refineTaskForReadiness({
  ...
  return NextResponse.json({ ok: true, refinement }, { status: 201 });
}
